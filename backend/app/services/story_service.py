...
def build_prompt(user_id: str, data: StoryRequest) -> str:
    ...
    characters = ", ".join([key.split("/")[-1].split(".")[0] for key in data.character_keys])
    settings = ", ".join([key.split("/")[-1].split(".")[0] for key in data.setting_keys])
    ...
    return (
        ...
        f"{name} is {personality} and loves {likes}. "
...
def generate_story(user_id: str, data: StoryRequest) -> StoryResponse:
    ...
    try:
        ...
        title = story_text.split("\n")[0] if story_text.lower().startswith("title") else "A Special Adventure"
        ...
        return StoryResponse(
            title="Error",
            story=f"Something went wrong: {str(e)}",
            character_keys=data.character_keys,
            setting_keys=data.setting_keys
        )
